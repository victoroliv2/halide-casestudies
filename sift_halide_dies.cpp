    ...
    for(int i = 1; i <= intervals; i++ )
      {
        ...
        dxx(x,y) = dog_pyr[o][i  ](x+1, y  ) + dog_pyr[o][i  ](x-1, y  ) - 2.0f * v;
        dyy(x,y) = dog_pyr[o][i  ](x,   y+1) + dog_pyr[o][i  ](x,   y-1) - 2.0f * v;
        dss(x,y) = dog_pyr[o][i+1](x,   y  ) + dog_pyr[o][i-1](x,   y  ) - 2.0f * v;
        dxy(x,y) = ( dog_pyr[o][i  ](x+1, y+1) - dog_pyr[o][i  ](x-1, y+1) - dog_pyr[o][i  ](x+1, y-1) + dog_pyr[o][i  ](x-1, y-1) ) / 4.0f;
        dxs(x,y) = ( dog_pyr[o][i+1](x+1, y  ) - dog_pyr[o][i+1](x-1, y  ) - dog_pyr[o][i-1](x+1, y  ) + dog_pyr[o][i-1](x-1, y  ) ) / 4.0f;
        dys(x,y) = ( dog_pyr[o][i+1](x,   y+1) - dog_pyr[o][i+1](x,   y-1) - dog_pyr[o][i-1](x,   y+1) + dog_pyr[o][i-1](x,   y-1) ) / 4.0f;

        DECL_FUNC(pc_det);
        pc_det(x,y) = dxx * dyy(x,y) - 2.0f * dxy(x,y);

        DECL_FUNC(pc_tr);
        pc_tr(x,y)  = dxx(x,y) + dyy(x,y);

        DECL_FUNC(invdet);
        invdet(x,y) = 1.0f/(  ( dxx(x,y) * (dyy(x,y) * dss(x,y) - dys(x,y) * dys(x,y)) )
                            - ( dxy(x,y) * (dxy(x,y) * dss(x,y) - dys(x,y) * dxs(x,y)) )
                            + ( dxs(x,y) * (dxy(x,y) * dys(x,y) - dyy(x,y) * dxs(x,y)) ));
        ...
        inv_dxx(x,y) = (invdet(x,y)) * (dyy(x,y) * dss(x,y) - dys(x,y) * dys(x,y));
        inv_dyy(x,y) = (invdet(x,y)) * (dxx(x,y) * dss(x,y) - dxs(x,y) * dxs(x,y));
        inv_dss(x,y) = (invdet(x,y)) * (dxx(x,y) * dyy(x,y) - dxy(x,y) * dxy(x,y));
        inv_dxy(x,y) = (invdet(x,y)) * (dxs(x,y) * dys(x,y) - dxy(x,y) * dss(x,y));
        inv_dxs(x,y) = (invdet(x,y)) * (dxy(x,y) * dys(x,y) - dxs(x,y) * dyy(x,y));
        inv_dys(x,y) = (invdet(x,y)) * (dxy(x,y) * dxs(x,y) - dxx(x,y) * dys(x,y));

        DECL_FUNC(interp);
        interp(c,x,y) = select(c == 0, inv_dxx(x,y) * dx(x,y) + inv_dxy(x,y) * dy(x,y) + inv_dxs(x,y) * ds(x,y),
                        select(c == 1, inv_dxy(x,y) * dx(x,y) + inv_dyy(x,y) * dy(x,y) + inv_dys(x,y) * ds(x,y),
                                       inv_dxs(x,y) * dx(x,y) + inv_dys(x,y) * dy(x,y) + inv_dss(x,y) * ds(x,y)));
        ...
        if (use_gpu())
          {
            ...
            inv_dxx.root().cudaTile(x,y,16,16);
            inv_dyy.root().cudaTile(x,y,16,16);
            inv_dss.root().cudaTile(x,y,16,16);
            inv_dxy.root().cudaTile(x,y,16,16);
            inv_dxs.root().cudaTile(x,y,16,16);
            inv_dys.root().cudaTile(x,y,16,16);
        ...
        else
          {
            ...
            dx.root().parallel(y).vectorize(x, 4);
            dy.root().parallel(y).vectorize(x, 4);
            ds.root().parallel(y).vectorize(x, 4);

            //hessian.root().vectorize(x, 4).parallel(y);
            ...
            inv_dxx.root().parallel(y).vectorize(x, 4);
            inv_dyy.root().parallel(y).vectorize(x, 4);
            inv_dss.root().parallel(y).vectorize(x, 4);
            inv_dxy.root().parallel(y).vectorize(x, 4);
            inv_dxs.root().parallel(y).vectorize(x, 4);
            inv_dys.root().parallel(y).vectorize(x, 4);
